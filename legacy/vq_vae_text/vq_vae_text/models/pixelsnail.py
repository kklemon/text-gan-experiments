...
class PixelSNAIL(nn.Module):
    def __init__(
        ...
        self.embed = nn.Embedding(n_class, embed_dim)
        ...
        self.horizontal = CausalConv2d(
            embed_dim, channel, [kernel // 2, kernel], padding='down'
        )
        self.vertical = CausalConv2d(
            embed_dim, channel, [(kernel + 1) // 2, kernel // 2], padding='downright'
        )
    ...
    def forward(self, input, condition=None, cache=None):
        ...
        input = self.embed(input).permute(0, 3, 1, 2)
