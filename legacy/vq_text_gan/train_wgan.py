    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for step, codes in enumerate(batches):
                ...
                code = code.to(device)
                code_one_hot = F.one_hot(code, num_classes=num_classes).transpose(1, 2).type(torch.float)

                z = torch.randn(code.size(0), args.latent_size, 1).to(device)

                # Optimize the discriminator
                fake_logits = G(z, extract_at_grow_index=current_grow_index)
                fake_probs = torch.softmax(fake_logits / 0.1, dim=1).detach()

                D_opt.zero_grad()

                loss_d, p_fake, p_real = loss_f.d_loss(code_one_hot, fake_probs)
                loss_d.backward()
                ...
                # Optimize generator
                fake_logits = G(z, extract_at_grow_index=current_grow_index)
                fake_probs = torch.softmax(fake_logits / 0.1, dim=1)

                G_opt.zero_grad()

                loss_g = loss_f.g_loss(code_one_hot, fake_probs)
                loss_g.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run-name', default='wgan')
