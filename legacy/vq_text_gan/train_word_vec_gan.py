...
def main(args):
    ...
    bpe = BPEmb(lang='de', vs=10000, dim=50, add_pad_emb=True)
    ...
    # spiegel model
    G = Generator(args.latent_size, [256, 256, 256, 128, 64], out_dim=inter_dim).to(device)
    D = UnetDiscriminator(64, max_channel=256, depth=5, in_dim=inter_dim).to(device)

    # G = Generator(args.latent_size, [256, 256, 256, 256], out_dim=inter_dim, attn_at=2).to(device)
    # D = UnetDiscriminator(256, max_channel=256, depth=4, in_dim=inter_dim, attn_at=2).to(device)
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for step, reals in enumerate(batches):
                ...
                #reals_embed += torch.normal(0, 0.1, size=reals_embed.shape, device=device)
                ...
                d_loss, p_real_g, p_real_l, p_fake_g, p_fake_l = loss_f.loss_d(reals_embed, fake_out.detach())
                d_loss.backward()
